...
class AgentClient:
    ...
    def invoke(
        ...
        agent_version: str,
        thread_id: str,
        tool_choice: dict[str, str],  # TODO: split supervisor and agent. their tool choice should be different
        messages: list[dict[str, str]], #TODO: get the last message from the list
        model: str | None = None,
        agent_config: dict[str, Any] | None = None,
    ) -> ChatMessage:
        """
        ...
        message = messages[-1]["content"][0]["text"] # Assuming the last message is human message
        request = UserInput(message=message)
        if thread_id:
            request.thread_id = thread_id
        if model:
